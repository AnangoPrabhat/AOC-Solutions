#replace s with your input
#sometimes it may provide a wrong answer, manually check for duplicates in the outputted list then run the area function again
...
def area(coords):
    ...
    del coords[-1]
    return s
def cv(x,y,dx,dy,nx,ny,grid):
    if (dx,dy)==(1,0):
        return grid[x][y] in 'SF7|' and grid[nx][ny] in 'SJL|'
    elif (dx,dy)==(-1,0):
        return grid[x][y] in 'SJL|' and grid[nx][ny] in 'SF7|'
    elif (dx,dy)==(0,-1):
        return grid[x][y] in 'SJ7-' and grid[nx][ny] in 'SFL-'
    elif (dx,dy)==(0,1):
        return grid[x][y] in 'SFL-' and grid[nx][ny] in 'SJ7-'
def ff(s):
    ...
    while q:
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nx, ny = x + dx, y + dy
            #print(x,y,dx,dy,nx,ny,check_valid(x,y,dx,dy,nx,ny,grid))
            if 0 <= nx < rows and 0 <= ny < cols and grid[nx][ny] != '.' and distance[nx][ny] == -1 and cv(x,y,dx,dy,nx,ny,grid):
    ...
    def getnext(pos):
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nx, ny = x + dx, y + dy
            if (nx,ny) in allps and cv(x,y,dx,dy,nx,ny,grid):
                l.append((nx,ny))
        return l
    t=getnext(st)
    print(st,t)
    ...
    for i in range(len(allps)):
        #print(allps2[-1],getnext(allps2[-1]))
        x,y=getnext(tau[-1])
        if x==tau[-2]:
            tau.append(y)
        else:
    ...
    su=0
    print(max(max(i) for i in distance))
    return max(max(i) for i in distance),tau
...
|F--J
LJ...'''
s='''L7L7F|7-77F7---LF7L7L7.L-7-F7.F-F-77.|.FJ7J7FF|.F-FF7FL..--77F-F--77FF7FFF77L|777F7F--F-FL7F-7F-7FJ7-FJ-77.F-|--F7-J7FJ7F.LF-|JF-FJ777|-|F77
